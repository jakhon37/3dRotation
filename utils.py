...
def make_scaled_cord(jsonfile, scale):
    with open(jsonfile, "r") as f: 
        coords_list =[]
        json_f = json.load(f)
        pr = json_f["parabola"]
        for i in (json_f["parabola"]):
            coords_list2.append([nt(i[0]*scale+420), int(i[1]*(-scale)+1300)]) #= [json_f["parabola"], json_f["parabola"]]
...
# pose_innfo = "./data/res/alphapose-results.json" #./data/txt/alphapose-results.json"
# path = './data/res/vis/'
...
def draw_guide_line(ball_center, pose_info, path):
    ...
    for idx, i in enumerate(json_f):
        ...
        frame = cv2.line(im3, start_point, end_point, color=(255, 50, 50 ), thickness=3)
